...
class model_training:
    ...
    def __init__(self, df, features, target="target", params_prop=0.25, test_size=0.33, hyperparams=None, 
           classifier="xgboost", eval_metric="recall", search_space=None,
           balance_data = None
           ):
    ...
    def _get_metrics(self):
        ...
        metrices = model_artifacts.get_metrics(self.model, self.X_test, self.y_test)

        predictions = model_artifacts.get_predictions(self.model, self.X_test, self.y_test)

        recall_precision_arrays = model_artifacts.get_prediction_arrays(self.model, self.X_test, self.y_test)
    ...
    def _feature_imp(self):
        ...
        try:
            ...
            feature_importance = model_artifacts.get_variableImportance(self.model, self.features) 
    ...
    def _split_data(self): 
        ...
        if self.balance_data == "both":

            print("splitting data init.....")
...
class plot_model_perf:
    ...
    def __init__(self, model, features, target="target", train_set=pd.DataFrame(), 
        ...
        for eval_set in self.eval_sets:
            ...
            if isinstance(data, pandas.core.frame.DataFrame) and not data.empty:
                ...
                predictions = model_artifacts.get_predictions(self.model, data[self.features], data[self.target])
                self.eval_sets_dic[eval_set]["predictions"] = predictions

                _metrices = model_artifacts.get_metrics(model, data[self.features], data[self.target])
                self.eval_sets_dic[eval_set]["_metrices"] = _metrices

                pred_array = model_artifacts.get_prediction_arrays(model, data[self.features], data[self.target])
                self.eval_sets_dic[eval_set]["pred_array"] = pred_array

        self.df_features = model_artifacts.get_variableImportance(self.model, self.features, returnType="df")
    ...
    def make_plots(self, display_metrics=True):
        ...
        model_plots.plot_comf_matrix(confMatrix, ax=fig_ax1, dataset=self.main_set)
        ...
        model_plots.plot_prediction(y_test, pred_class, pred_prob, ax=fig_ax4, dataset=self.main_set)
        ...
        for i, eval_set in enumerate(self.eval_sets_dic):
            ...
            model_plots.plot_roc_curve(trP, trN, area_roc, ax=fig_ax2, color=color, label=eval_set)
            model_plots.plot_rp_curve(recall_curve, precision_curve, ax=fig_ax3, color=color, label=eval_set)

        model_plots.plot_lift_curve(y_test, pred_prob, ax=fig_ax5, dataset=self.main_set)

        if not self.df_features.empty:
            
            fig_ax6 = fig.add_subplot(spec2[1,:])
            model_plots.plot_feature_imp(self.df_features, ax=fig_ax6)
...
def hyper_parameter_tunning(features, labels, classifier="xgboost", eval_metric="recall", search_space=None):
...
class model_artifacts:
    ...
    def get_metrics(model, X_test, y_test):
        ...
        predictions = model_artifacts.get_predictions(model, X_test, y_test)
...
class model_plots:
    ...
    @staticmethod
    def plot_comf_matrix(cm, ax, dataset="test"):
    ...
    @staticmethod
    def plot_roc_curve(trP, trN, area_roc, ax, color="orange", label="test"):
    ...
    @staticmethod
    def plot_rp_curve(rcl, prc, ax, color="orange", label="test"):
    ...
    @staticmethod
    def plot_prediction(y_test, pred_class, pred_prob, ax, dataset="test"):
    ...
    @staticmethod
    def plot_feature_imp(df_features, ax):
    ...
    @staticmethod
    def plot_lift_curve(y_test, pred_prob, ax=None, output="plot", dataset="test"):
